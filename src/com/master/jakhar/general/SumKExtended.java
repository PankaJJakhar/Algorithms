...
public class SumKExtended {
	...
	public static void main(String[] args) {
		...
		input = new int[] { 2, 2 };
		System.out.println("Test case 6 for Sum = 4>>>");
		printOutput(findSum(input, 4));
		System.out.println();

		input = new int[] { 2, 2, 2 };
		System.out.println("Test case 7 for Sum = 4>>>");
		printOutput(findSum(input, 4));
		System.out.println();

		input = new int[] { 4, 6, 4, 6 };
		System.out.println("Test case 8 for Sum = 10>>>");
		printOutput(findSum(input, 10));
		System.out.println();

		input = new int[] { 4, 6, 6, 6 };
		System.out.println("Test case 9 for Sum = 10>>>");
		printOutput(findSum(input, 10));
		System.out.println();
	}
	...
	private static void printOutput(ArrayList<Index> indicesList) {
		...
		for (int i = 0; i < size; i++) {
			...
			System.out.printf("Indices Pair #%d>>>>>\n", i + 1);
	...
	public static ArrayList<Index> findSum(int[] input, int sum) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (valueToIndices.containsKey(sum - currentValue)) {
				...
				/** Get stored indices list from Lookup table. */
				ArrayList<Integer> storedIndices = valueToIndices.get(sum
						- currentValue);

				/** Now create new Index pairs with this current index. */
				int storedIndicesSize = storedIndices.size();
				for (int indexCount = 0; indexCount < storedIndicesSize; indexCount++) {
					indicesList.add(new Index(currentIndex, storedIndices
							.get(indexCount)));
				}
			}
			...
			if (valueToIndices.containsKey(currentValue)) {
				ArrayList<Integer> indexList = valueToIndices.get(currentValue);
				indexList.add(i);
				valueToIndices.put(currentValue, indexList);
			} else {
